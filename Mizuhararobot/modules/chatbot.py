...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_chat:
        ...
        msg.reply_text("AI successfully enabled for this chat nyann~~!")
        message = (
        ...
        msg.reply_text("AI is already enabled for this chat nyan~~!")
        return ""
...
def remove_chat(update: Update, context: CallbackContext):
    ...
    is_chat = sql.is_chat(chat.id)
    if not is_chat:
        msg.reply_text("AI isn't enabled here in the first place nyan~~!")
        return ""
    else:
        sql.rem_chat(chat.id)
        msg.reply_text("AI disabled successfully nyan~~!")
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#AI_DISABLED\n"
            f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
